...
class RunningSushiBuffer:
    ...
    def __init__(self, dim):
        ...
        self.pranzoInCorso = True
        self.clienti = []
    
    def getStatoPranzo(self):
        with self.lock:
            return self.pranzoInCorso

    def rimuoviCliente(self,c):
        with self.lock:
            self.clienti.remove(c)
            print (f"Size clienti: {len(self.clienti)}")
            if (len(self.clienti)==0):
                self.pranzoInCorso=False
                self.condition.notifyAll()
    
    def aggiungiCliente(self,c):
        with self.lock:
            self.pranzoInCorso=True
...
class Cuoco(Thread):
    ...
    def run(self):
        while(self.d.getStatoPranzo() or len(self.d.clienti)!=0):
            sleep(0.5 * random())
            self.iterazioni -= 1
...
class Cliente(Thread):
    ...
    def __init__(self, d : RunningSushiBuffer, pos : int):
        super().__init__()
        self.coseCheVoglioMangiare = randint(1,4)
        self.d = d
        self.pos = pos

    def run(self):
        self.d.aggiungiCliente(self.ident)
        ...
        self.d.rimuoviCliente(self.ident)
