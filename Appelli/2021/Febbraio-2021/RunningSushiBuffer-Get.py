...
class RunningSushiBuffer:
    ...
    def waitForGet (self, pos : int, piatto):
        with self.lock:
            while self.theBuffer[self._getRealPosition(pos)] != piatto:
                self.condition.wait()
            palluzza = self.theBuffer[self._getRealPosition(pos)]
            self.theBuffer[self._getRealPosition(pos)] = None
            return palluzza
...
class Cliente(Thread):
    ...
    def run(self):
        while(self.coseCheVoglioMangiare > 0):
            ...
            print ( f"Il cliente {self.ident} aspetta cibo {self.piatti[randPiatto]}")
            print ( f"Il cliente {self.ident} mangia <{self.d.waitForGet(self.pos,self.piatti[randPiatto])}>")
